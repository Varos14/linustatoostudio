import { StudentTokenData } from '@/lib/jwt'; // Assuming you have this for DB lookups
import { sendEmail } from '@/lib/email'; // Assuming you have this for sending emails
...
class PesapalService {
  ...
  private async getAccessToken(): Promise<string> {
    ...
    try {
      ...
      this.tokenExpiry = Date.parse(data.expiryDate) - (5 * 60 * 1000); // Set expiry 5 mins before actual
  ...
  /**
   * 💡 ADDED: Registers the IPN URL with Pesapal and caches the returned ID.
  ...
  async submitOrderRequest(paymentData: PaymentRequest): Promise<PaymentResponse> {
    try {
      ...
      const notification_id = await this.getIpnId(); // 💡 ADDED: Get the IPN ID before submitting
      ...
      const response = await fetch(endpoint, {
        ...
        body: JSON.stringify({
          ...paymentData,
          notification_id: notification_id, // 🔧 MODIFIED: Always include the dynamic notification_id
        })
      });
